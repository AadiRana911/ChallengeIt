...
class Chellenges extends Component {
  ...
  componentDidMount = () => {
    ...
    new Promise((rsl, rej) => {
      this.setState({loading: true}, () => {
        this.props.nearbyUsers(this.props.token, rsl, rej);
      });
    })
      .then((res) => {
        this.grabChallenges();
            ...
            new Promise((rsl, rej) => {
              ...
              .then((res) => {
                this.props.allchallenges &&
                  this.setState({
                    videos: this.props.allchallenges.map((item) => {
                      return {
                        ...item,
                        loading: true,
                        muted: false,
  ...
  handleHideVideo = () => {
    ...
    this.setState(
      ...
      () => {
        ...
        // const formData = new FormData();
        // formData.append('challange_id', this.state.challangeId);
        // new Promise((rsl, rej) => {
        //   this.props.hideVideo(this.props.token, formData, rsl, rej);
        // })
        //   .then((res) => {
        //     console.log(res);
        //   })
  ...
  actualDownload = () => {
    console.log(this.state.vid, this.state.challengeId);
    const downloadDest = `${RNFS.DocumentDirectoryPath}/${
      (Math.random() * 1000) | 0
    }.mp4`;
    const formUrl = this.state.vid;
          ...
          CameraRoll.save(downloadDest)
            .then(() => {
              ...
              this.setState(
                ...
                () => {
                  const params = new FormData();
                  params.append('challenge_id', this.state.challengeId);
                  new Promise((rsl, rej) => {
                    this.props.addDownload(this.props.token, params, rsl, rej);
                  })
                    .then((res) => {})
                    .catch((err) => {});
                },
              ...
              <TouchableWithoutFeedback
                onPress={() => {
                  this.setState(
                    ...
                    () => {
                      this.props.navigation.navigate('User', {
                        uid: this.props.user && this.props.user.u_id,
                      });
                    },
                  );
                }}>
              ...
              <Animated.View style={{width: this.state.searchWidth}}>
                <TextInput
                  ...
                  style={{
                    width: '70%',
                    borderRadius: 3,
                    borderBottomWidth: this.state.searchState ? 0.8 : 0,
                    borderColor: this.state.searchState ? '#ddd' : '#fff',
                    paddingLeft: 10,
                    color: 'white',
                  }}
                />
        ...
        {this.state.showAvatar && (
          <View
            ...
            {this.state.singleUser && (
              <Image
                source={{
                  uri: `${BASE_URL}${this.state.singleUser.dp}`,
                }}
                style={{
                  height: 90,
                  width: 90,
            ...
            <Text
              ...
              {this.state.singleUser &&
                this.state.singleUser.f_name +
                  ' ' +
                  this.state.singleUser.l_name}
            </Text>
...
const mapStateToProps = (state) => {
  const {nearby, allchallenges, playlists, dp} = state.app;
  const {token, user} = state.auth;
  return {nearby, token, user, allchallenges, playlists, dp};
};
