...
class ProfileScreen extends Component {
  ...
  grabChallenges = () => {
    const challenge_id =
      this.props.route.params && this.props.route.params.challenge_id;
    ...
    try {
      ...
      new Promise((rsl, rej) => {
        this.props.getSingleChallenges(formData, this.props.token, rsl, rej);
      })
        .then((res) => {
          console.log(this.props.singleChallenge);
          this.props.singleChallenge &&
            this.setState({
              videos: this.props.singleChallenge.map((item) => {
                return {
                  ...item,
                  loading: true,
                  muted: false,
  ...
  render() {
    const {nudity, cheat, voilence, other, singleUser} = this.state;
    return (
              ...
              return (
                <TouchableWithoutFeedback
                  ...
                  onPress={() => {
                    this.props.navigation.navigate('Feed', {
                      from: 'yes',
                      chellenge_id: item.challenge_id,
                    });
                  }}>
                  <Image
...
const mapStateToProps = (state) => {
  const {nearby, singleChallenge, playlists, responses} = state.app;
  const {token, user} = state.auth;
  return {nearby, token, user, singleChallenge, playlists, responses};
};
